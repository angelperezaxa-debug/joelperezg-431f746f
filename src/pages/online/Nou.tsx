      ...
      <main className="menu-screen min-h-screen flex flex-col items-center px-5 py-8">
        <div className="w-full max-w-md flex flex-col gap-5">
          ...
          <header className="text-center">
            <h1 className="font-title font-black italic text-gold text-3xl pr-2 text-center">Crear taula online</h1>
            <p className="mt-1 text-sm text-muted-foreground">Cal configurar el teu nom abans de crear la taula</p>
          </header>
          <Button
            size="lg"
            variant="outline"
          ...
          >
            <Settings className="w-4 h-4 mr-2" /> Anar a Ajustes
          </Button>
        </div>
      </main>
    );
  ...
  return (
    <main className="menu-screen min-h-screen flex flex-col items-center justify-center px-5 py-8">
      <div className="flex flex-col items-center gap-3">
        <Loader2 className="w-6 h-6 animate-spin text-primary" />
        <p className="text-sm text-muted-foreground">Creant taula…</p>
        {error && (
          <>
            <p className="text-xs text-destructive text-center max-w-xs">{error}</p>
            <Button size="sm" variant="outline" onClick={() => navigate("/")}>Tornar a inici</Button>
          </>
        )}
      </div>
    </main>
